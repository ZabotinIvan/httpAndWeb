...
public class Processor implements Runnable {
    ...
    private static final byte[] REQUEST_LINE = {'\r', '\n'};
    private static final byte[] HEADERS = {'\r', '\n', '\r', '\n'};
    ...
    private int processRequestLine(byte[] buffer, int read, BufferedOutputStream out) throws URISyntaxException {
        // ищем request line
        int requestLineEnd = indexOf(buffer, REQUEST_LINE, 0, read);
        ...
        return requestLineEnd + REQUEST_LINE.length;
    }
    ...
    private boolean processRequestHeaders(byte[] buffer, int read, int headersStart,
        ...
        final var headersEnd = indexOf(buffer, HEADERS, headersStart, read);
        if (headersEnd == -1) {
            ResponseUtils.sendBadRequest(out);
            return false;
        }
    ...
    private void processRequestBody(BufferedInputStream in) throws IOException {
        in.skip(HEADERS.length);
        // вычитываем Content-Length, чтобы прочитать body
        Optional<String> contentLength = extractHeader(request.getHeaders());
    ...
    private Optional<String> extractHeader(List<String> headers) {
